...
export const getGenres = createAsyncThunk("flixxit/genres", async () => {
  const {
    ...
    "https://api.themoviedb.org/3/genre/movie/list?api_key=b3694e918def9f3c4a9a5b1807c0feb6"
  );
  return genres;
});
...
export const getUsersLikedMovies = createAsyncThunk(
  ...
  async (email) => {
    const {
      data: { movies },
    } = await axios.get(`https://backend-hlqp.onrender.com/api/user/liked/${email}`);
    return movies;
  }
);
...
export const removeMovieFromLiked = createAsyncThunk(
  ...
  async ({ movieId, email }) => {
    const {
      data: { movies },
    } = await axios.put("https://backend-hlqp.onrender.com/api/user/remove", {
      email,
      movieId,
    });
    return movies;
